...
async function syncWithTMDB(localMedia) {
    console.log('Starting TMDB data synchronization in batches...');
    const BATCH_SIZE = 10;
    let updatedMedia = [];
    ...
    for (let i = 0; i < localMedia.length; i += BATCH_SIZE) {
        ...
        const syncPromises = batch.map(async (item) => {
            if (!item.tmdb_id || !item.type) {
                return item; // Skip items with missing data
            }
            ...
            try {
                ...
                if (!response.ok) {
                    console.warn(`Could not fetch TMDB data for ID ${item.tmdb_id}. Status: ${response.status}. Skipping.`);
                    return item;
                }
                ...
                if (Object.keys(updates).length > 0) {
                    console.log(`Updating '${item.title}' (ID: ${item.id})`);
                    const { error } = await supabase.from('media').update(updates).eq('id', item.id);
                    if (error) {
                        console.error(`Error updating Supabase for item ID ${item.id}:`, error);
                        return item;
                    }
                    return { ...item, ...updates };
                }
                ...
                console.error(`Error during TMDB sync for item ID ${item.id}:`, error);
                return item;
            }
        });
